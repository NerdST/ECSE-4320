...
// Scalar SAXPY
template <typename T>
void saxpy_scalar(T a, const T *x, T *y, size_t n, size_t stride = 1)
{
...
template <typename T>
std::array<T, 2> run_experiment(T a, size_t n, size_t stride, bool aligned, bool verbose = true)
{
  ...
  saxpy_scalar<T>(a, x, y, n, stride); // Warmup
  auto start = std::chrono::high_resolution_clock::now();
  saxpy_scalar<T>(a, x, y, n, stride);
  ...
  if (verbose)
  {
    std::cout << "N=" << n << " | stride=" << stride << " | aligned=" << aligned
              << " | time=" << seconds << " s | GFLOP/s=" << gflops << std::endl;
  }
  free_mem(x, aligned);
  free_mem(y, aligned);
  return {static_cast<T>(gflops), static_cast<T>(seconds)};
}
...
void run_and_write(const SaxpyConfig &cfg, const std::vector<size_t> &sizes, int repeats, std::ofstream &csv)
{
  for (size_t val : sizes)
  {
    ...
    {
      auto result = run_experiment<T>(static_cast<T>(2.5), val, cfg.stride, cfg.aligned, false);
...
int main(int argc, char **argv)
{
  ...
  csv << "datatype,n,stride,aligned,time_s,gflops\n";
  int repeats = 3;
  if (cfg.sweep)
  {
    if (cfg.use_float32)
  ...
  else
  {
    if (cfg.use_float32)
    {
      auto result = run_experiment<float>(2.5f, cfg.n, cfg.stride, cfg.aligned, true);
    ...
    {
      auto result = run_experiment<double>(2.5, cfg.n, cfg.stride, cfg.aligned, true);
